...
# Used to store detection results for each frame
results = []

# Specify the video file
video_file = '/Users/yukihan/Desktop/final/Emotion-recognition/sample/whole/zhong/scared-surprise.mp4'

camera = cv2.VideoCapture(video_file)

# Get the frame rate of the video
frame_rate = int(camera.get(cv2.CAP_PROP_FPS))
frame_count = 0  # Used to count the current frame number

# Calculate the number of frames corresponding to 30 seconds
max_frame_count = 30 * frame_rate

# Extract video frames from the first 30 seconds
while frame_count < max_frame_count:
    ...
    if len(faces) > 0:
        ...
        # Calculate the current frame's time (in seconds)
...
# Triple the size of the frame
    ...
    frame_count += 1  # Used to store detection results for each frame
    ...
    # Name the output file based on the video name and generate a graph and table for the 30-second data
    video_file_name = os.path.basename(video_file).split('.')[0]  # Get the video file name without the extension

    # Filter the data, keeping only records from the first 30 seconds
    emotion_df = emotion_df[emotion_df['Time(s)'] <= 30]

    # Print emotion_df for debugging
    print("emotion_df data:")
    print(emotion_df)

    # Check if there are any emotion probability outputs
    if emotion_df.empty:
        print("No emotion data was detected.")
    else:
        # Print emotion data from the first 30 seconds
        print("Emotion data in first 30 seconds:")
        print(emotion_df)
        ...
        # Set the transparency for emotions
        ...
        for idx, emotion in enumerate(EMOTIONS):
            ...
            # Get the time and probability data
        ...
        # If time_data is not empty
            if not time_data.empty:
                ...
                if emotion == 'happy':
                    ...
                    # Convert the emotion color to Matplotlib's 0-1 range RGB values
                ...
                plt.scatter(time_data, probability_data_smooth, label=emotion,
                            ...
                            alpha=alphas[idx], s=50)  # s parameter sets the size of the points

        # Set the title, labels, and legend
        plt.title(f'Emotion Tracking for {video_file_name} (First 30 seconds with Scatter)')
        plt.xlabel('Time (s)')
        plt.ylabel('Emotion Probability')
        plt.ylim([0, 1])  # Unify the y-axis range
        plt.legend()
        plt.grid(True)

        # Save the scatter plot
        plt.savefig(f"{video_file_name}_emotion_tracking_scatter_30s.png")
    ...
    for idx, emotion in enumerate(EMOTIONS):
        ...
        # If time_data is not empty
        if not time_data.empty:
            # If the emotion is 'happy', set the color to adjusted yellow
            if emotion == 'happy':
        ...
        # Set the title, labels, and legend
    plt.title(f'Emotion Tracking for {video_file_name} (First 30 seconds with Lines)')
    plt.xlabel('Time (s)')
    plt.ylabel('Emotion Probability')
    plt.ylim([0, 1])  # Unify the y-axis range
    plt.legend()
    plt.grid(True)
        ...
        # Save the line graph
    plt.savefig(f"{video_file_name}_Emotion_tracking_lines_30s.png")
    ...
    brightness_factor = 0.7  # A value less than 1, such as 0.7, means reducing brightness by 30%

    # Move the RGB colors closer to gray, reducing saturation
    def adjust_brightness_and_saturation(color, saturation_factor=0.3, brightness_factor=0.7):
        ...
        color = np.array(color) * brightness_factor  # Adjust brightness first
        return (np.array(color) * (1 - saturation_factor) + gray * saturation_factor).astype(int)

    # Adjust the colors in emotion_colors and apply desaturation
    colors = [adjust_brightness_and_saturation(emotion_colors[emotion], desaturation_factor, brightness_factor) 
        ...
        # Adjust the colors to include transparency
    colors_with_alpha = [(r/255, g/255, b/255, 0.55) for r, g, b in colors]  # Set transparency to 0.55

    # Custom autopct function to set percentage text to white
    ...
    # Use the autopct parameter to customize the percentage display
    patches, texts, autotexts = plt.pie(emotion_percentages, labels=emotion_totals.index, autopct=autopct_format,
                                        ...
                                        textprops={'color': 'black'})  # First, set the label to black

    # Set the percentage text to white and bold
    for autotext in autotexts:
        autotext.set_color('white')  # Set percentage text to white
        autotext.set_fontsize(12)    # Set font size
        autotext.set_fontweight('bold')  # Bold the percentage font

        # # Add external label lines
        # plt.gca().add_artist(plt.Circle((0, 0), 0.70, fc='white'))  # Inner circle creates a "donut" effect

    # Set the title
    plt.title(f'Emotion Distribution for {video_file_name} (First 30 seconds)')

    # Save the pie chart
    plt.savefig(f"{video_file_name}_emotion_pie_chart_30s.png")

    # Clear the plot cache
    plt.clf()

    # Save the results as an Excel file, with the file name based on the video name.
    emotion_df.to_excel(f"{video_file_name}_emotion_tracking_30s.xlsx", index=False)
